...
CPU_INT32U  const  MB_TotalRAMSize = sizeof(MB_RTU_Freq)
                                   ...
                                   + MODBUS_CFG_MAX_CH*sizeof(MODBUS_CH);
#else
CPU_INT32U  const  MB_TotalRAMSize = MODBUS_CFG_MAX_CH*sizeof(MODBUS_CH);
#endif
...
MODBUS_CH *MB_ChTblPrt[MODBUS_CFG_MAX_CH];
CPU_INT08U      MB_ChCtr;
CPU_INT16U      MB_RTU_Freq;
...
#endif
                                    /* Save Modbus channel number in data structure       */ 
    for (ch = 0; ch < MODBUS_CFG_MAX_CH; ch++) {                /* Initialize default values                          */
        /* Allocate a block and release the block memory.  */
     status = tx_byte_allocate(&byte_pool_0, (VOID **) &pointer, MB_ChSize, TX_NO_WAIT);
        if (status != TX_SUCCESS){
#if     (RTT_PRINTF_EN == DEF_ENABLED)
          SEGGER_RTT_printf(0, "Modbus ch: fail %u.\r\n", ch);
#endif
          Error_Handler();
        }
#if     (RTT_PRINTF_EN == DEF_ENABLED)
          SEGGER_RTT_printf(0, "Modbus ch: Success %u.\r\n", ch);
#endif
        pch = (MODBUS_CH*)pointer;        
        pch->Ch            = ch;
        pch->NodeAddr      = 1;
...
//        pch->RTU_TimeoutEn = DEF_TRUE;
    ...
    for (ch = 0; ch < MODBUS_CFG_MAX_CH; ch++) { 
      pch= MB_ChTblPrt[ch];
      SEGGER_RTT_printf(0, "Modbus ch: %u.\r\n", pch->Ch);      
    }
#endif
...
// 释放内存
   for (int ch = 0; ch < MODBUS_CFG_MAX_CH; ch++){
      tx_byte_release((VOID *) MB_ChTblPrt[ch]);
    }
...
MODBUS_CH  *MB_CfgCh (CPU_INT08U  node_addr,
                      ...
                      UART_HandleTypeDef  *port_nbr,
                      CPU_INT16U RTU_Timeout,
                      CPU_INT08U  wr_en)
{
    MODBUS_CH   *pch;
#if (MODBUS_CFG_RTU_EN == DEF_ENABLED)
...
//    pch->TxBufPtr = &pch->TxBuf[0];
    ...
    status = HAL_UART1_Transmit_DMA(&huart1,&pch->TxBuf[0],pch->TxBufByteCtr);
#if     (RTT_PRINTF_EN == DEF_ENABLED)
      SEGGER_RTT_printf(0, "Modbus tx ctr:%u.\r\n", pch->TxCtr); 
#endif
    if (status != HAL_OK){
#if     (RTT_PRINTF_EN == DEF_ENABLED)
      SEGGER_RTT_printf(0, "HAL_UART_Transmit_DMA Error:%u.\r\n", status); 
        tx_thread_sleep(4u);
//        continue;
#endif
         Error_Handler();
