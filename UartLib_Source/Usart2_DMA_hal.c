...
static void User_UART2_DMA_SetConfig(DMA_HandleTypeDef *hdma, uint32_t SrcAddress, uint32_t DstAddress, uint32_t DataLength);
...
HAL_StatusTypeDef User_UART2_DMA_Tx_Start_IT(DMA_HandleTypeDef *hdma, uint32_t SrcAddress, uint32_t DstAddress, uint32_t DataLength)
{
  ...
  if(HAL_DMA_STATE_READY == hdma->State)
  {
    ...
    User_UART2_DMA_SetConfig(hdma, SrcAddress, DstAddress, DataLength);
...
void User_USART2_DMA_RX_IRQHandler(DMA_HandleTypeDef *hdma)
{
  ...
  if (((flag_it & (DMA_FLAG_HT1 << hdma->ChannelIndex)) != RESET) && ((source_it & DMA_IT_HT) != RESET))
  {
    ...
    User_UART2_ReceiveDMA_Callback(hdma);
  }
...
if (((flag_it & (DMA_FLAG_TC1 << hdma->ChannelIndex)) != RESET) && ((source_it & DMA_IT_TC) != RESET))
  {
      ...
      __HAL_UNLOCK(hdma);
      /* Transfer complete callback */
      User_UART2_DMATransmitCplt(hdma);
  }
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef User_UART2_DMA_Abort_IT(DMA_HandleTypeDef *hdma)
{  
  HAL_StatusTypeDef status = HAL_OK;
